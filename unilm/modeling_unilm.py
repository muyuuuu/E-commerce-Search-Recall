...
class UnilmModel(UnilmPreTrainedModel):
    ...
    def get_extended_attention_mask(self, input_ids, token_type_ids,
                                    attention_mask):
...
class LabelSmoothingLoss(_Loss):
    def __init__(self,
        ...
        super(LabelSmoothingLoss, self).__init__(size_average=size_average,
                                                 reduce=reduce,
                                                 reduction=reduction)
    ...
    def forward(self, output, target):
        ...
        return F.kl_div(output, model_prob.type_as(output),
                        reduction='none').view(batch_size, num_pos, -1).sum(2)
...
class UnilmForLM(UnilmPreTrainedModel):
    ...
    def forward(self,
        ...
        def gather_seq_out_by_pos(seq, pos):
            return torch.gather(seq, 1,
                                pos.unsqueeze(2).expand(-1, -1, seq.size(-1)))

        def gather_seq_out_by_pos_average(seq, pos, mask):
            batch_size, max_token_num = pos.size(0), pos.size(-1)
            pos_vec = torch.gather(
                seq, 1,
                pos.view(batch_size, -1).unsqueeze(2).expand(
                    -1, -1, seq.size(-1))).view(batch_size, -1, max_token_num,
                                                seq.size(-1))
            mask = mask.type_as(pos_vec)
            pos_vec_masked_sum = (pos_vec *
                                  mask.unsqueeze(3).expand_as(pos_vec)).sum(2)
            return pos_vec_masked_sum / mask.sum(
                2, keepdim=True).expand_as(pos_vec_masked_sum)
        ...
        sequence_output_masked = gather_seq_out_by_pos(sequence_output,
                                                       masked_pos)
        prediction_scores_masked = self.cls(sequence_output_masked)

        if self.crit_mask_lm_smoothed:
            masked_lm_loss = self.crit_mask_lm_smoothed(
                F.log_softmax(prediction_scores_masked.float(), dim=-1),
                masked_lm_labels)
        else:
            masked_lm_loss = self.crit_mask_lm(
                prediction_scores_masked.transpose(1, 2).float(),
                masked_lm_labels)
        masked_lm_loss = loss_mask_and_normalize(masked_lm_loss.float(),
                                                 masked_weights)
        ...
        if next_sentence_label is None:
            ...
            next_sentence_loss = self.crit_next_sent(
                seq_relationship_score.view(-1, self.num_labels).float(),
...
class UnilmForSeq2Seq(UnilmPreTrainedModel):
    ...
    def extract(self, input_ids, token_type_ids, attention_mask):
        out, __ = self.bert(input_ids,
                            token_type_ids,
                            attention_mask,
                            output_all_encoded_layers=True)
        first = out[1].transpose(1, 2)
...
class UnilmForSeq2SeqDecode(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask):
        if self.search_beam_size > 1:
            return self.beam_search(input_ids, token_type_ids, position_ids,
                                    attention_mask)
        ...
        while next_pos < output_length:
            ...
            curr_attention_mask = attention_mask[:, start_pos:next_pos +
                                                 1, :next_pos + 1]
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids,
                    attention_mask):
        input_shape = list(input_ids.size())
        batch_size = input_shape[0]
        input_length = input_shape[1]
        ...
        while next_pos < output_length:
            ...
            last_hidden = new_encoded_layers[-1][:, -1:, :]
            prediction_scores = self.cls(last_hidden)
            log_scores = torch.nn.functional.log_softmax(prediction_scores,
                                                         dim=-1)
            ...
            if len(total_scores) == 0:
                ...
                last_eos = torch.reshape(beam_masks[-1],
                                         [batch_size * K, 1, 1])
                last_seq_scores = torch.reshape(total_scores[-1],
                                                [batch_size * K, 1, 1])
            ...
            def select_beam_items(x, ids):
                ...
                assert x_rank >= 2
                if id_rank < x_rank:
                    ids = torch.reshape(ids,
                                        id_shape + [1] * (x_rank - id_rank))
            ...
            if prev_encoded_layers is None:
                prev_encoded_layers = [
                    first_expand(x[:, :-1, :]) for x in new_encoded_layers
                ]
            else:
                prev_encoded_layers = [
                    torch.cat((x[0], x[1][:, :-1, :]), dim=1)
            ...
            if self.forbid_duplicate_ngrams:
                ...
                if is_first:
                    ...
                    for b in range(batch_size):
                        for k in range(K):
                            new_partial_seqs.append(partial_seqs[ptrs[b][k] +
                                                                 b * K] +
                                                    [wids[b][k]])
                ...
                def get_dup_ngram_candidates(seq, n):
                    ...
                    if len(seq) < n:
                        return []
                    tail = seq[-(n - 1):]
                    if self.forbid_ignore_set and any(
                            tk in self.forbid_ignore_set for tk in tail):
                        return []
                    for i in range(len(seq) - (n - 1)):
                        ...
                        if (not mismatch) and not (
                                self.forbid_ignore_set and
                            (seq[i + n - 1] in self.forbid_ignore_set)):
                            cands.add(seq[i + n - 1])
                ...
                if len(partial_seqs[0]) >= self.ngram_size:
                    ...
                    if max(len(x) for x in dup_cands) > 0:
                        if buf_matrix is None:
                            ...
                            buf_matrix = np.zeros((batch_size * K, vocab_size),
                                                  dtype=float)
                        else:
                            buf_matrix.fill(0)
                        ...
                        forbid_word_mask = torch.tensor(buf_matrix,
                                                        dtype=log_scores.dtype)
                        forbid_word_mask = torch.reshape(
                            forbid_word_mask,
                            [batch_size * K, 1, vocab_size]).cuda()
                    else:
                        forbid_word_mask = None
            next_pos += 1
        ...
        for k in ('pred_seq', 'scores', 'wids', 'ptrs'):
            ...
            traces[k] = _pad_sequence(ts_list, output_length,
                                      padding_value=0).to(input_ids.device)
