...
class Pipeline():
    ...
    def get_masked_pos(self,
        ...
        for i, sp in enumerate(span_list):
            sp_st, sp_end = sp
            if (sp_end - sp_st == 1) and (tokens[sp_st].endswith('CLS]')
                                          or tokens[sp_st].endswith('SEP]')):
                special_pos.add(i)
            else:
                if mask_segment:
                    if ((i < segment_index) and
                        ('a' in mask_segment)) or ((i > segment_index) and
                                                   ('b' in mask_segment)):
                        cand_pos.append(i)
                else:
                    cand_pos.append(i)
        ...
        for i_span in cand_pos:
            ...
            if self.skipgram_size_geo_list:
                ...
                rand_skipgram_size = np.random.choice(
                    ...
                    1,
                    p=self.skipgram_size_geo_list)[0] + 1
            else:
                if add_skipgram and (self.skipgram_prb > 0) and (
                        self.skipgram_size >= 2) and (rand() <
                                                      self.skipgram_prb):
                    rand_skipgram_size = min(randint(2, self.skipgram_size),
                                             len(span_list) - i_span)
            ...
            if self.mask_whole_word:
                ...
                st_span, end_span = _expand_whole_word(tokens, st_span,
                                                       end_span)
            ...
            for sp in range(st_span, end_span):
                for mp in range(span_list[sp][0], span_list[sp][1]):
                    if not (skip_pos and (mp in skip_pos)) and (
                            mp not in special_pos
                    ) and not (protect_range and
                               (protect_range[0] <= mp < protect_range[1])):
                        masked_pos.add(mp)
...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        assert len(self.bi_uni_pipeline) == 4
    ...
    def gene_query(self, title):
        a = rand()
        query = None
        if a < 0.4 or len(title) >= 45:
            title_len = len(title)
            query_len = randint(int(title_len * .2), int(title_len * .4))
            start = randint(0, title_len - query_len)
            query = title[start:start + query_len]
        elif 0.4 <= a < 0.6:
            query = title[:4] + title[-4:]
        elif 0.6 <= a < 0.8:
            query = title[:8]
        else:
            query = title[-8:]
    ...
    def read_data(self, file, tokenizer):
        with open(file, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            for line in lines:
                line = line.strip().split("\t")
                self.ex_list.append((line[1], self.gene_query(line[1])))
                # src_tk = tokenizer.tokenize(sample["src_text"])
                # tgt_tk = tokenizer.tokenize(sample["tgt_text"])
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __call__(self, instance):
        ...
        tokens_a, tokens_b = truncate_tokens_pair(tokens_a, tokens_b,
                                                  self.max_len)
        # Add Special Tokens
        ...
        for i, tk in enumerate(tokens):
            ...
            if (i >= len(tokens_a) + 2) and (tk != '[CLS]'):
                cand_pos.append(i)
            elif self.mask_source_words and (i < len(tokens_a) + 2) and (
                    tk != '[CLS]') and (not tk.startswith('[SEP')):
                cand_pos.append(i)
            else:
                special_pos.add(i)
        shuffle(cand_pos)
        ...
        for pos in cand_pos:
            ...
            def _expand_whole_word(st, end):
                ...
                while (new_st >= 0) and tokens[new_st].startswith('##'):
                    new_st -= 1
                while (new_end <
                       len(tokens)) and tokens[new_end].startswith('##'):
                    new_end += 1
                return new_st, new_end

            if (self.skipgram_prb > 0) and (self.skipgram_size >= 2) and (
...
class Preprocess4Seq2seqDecode(Pipeline):
    ...
    def __call__(self, instance):
        ...
        assert len(padded_tokens_a) == max_a_len + 2
        max_len_in_batch = min(self.max_tgt_length + max_a_len + 2,
                               self.max_len)
        tokens = padded_tokens_a
        segment_ids = [4] * (len(padded_tokens_a)) + [5] * (
